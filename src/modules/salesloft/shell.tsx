...
export function SalesloftShell({ onBack }: Props) {
  ...
  if (selectedCategory !== null) {
    ...
    const categoryCommands = salesloftModule.commands.filter((cmd) => cmd.category === category.name);
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Box flexDirection="column" marginBottom={1}>
        ...
        <Text color="gray">• Total Commands: {salesloftModule.totalCommands}</Text>
        <Text color="gray">• Categories: {salesloftModule.categories.length}</Text>
