...
export function SmartLeadShell({ onBack }: Props) {
  ...
  useInput((input, key) => {
    ...
    if (selectedCategory === null && !showWelcome && /^[1-6]$/.test(input)) {
      ...
      if (smartleadModule.categories[categoryIndex]) {
  ...
  if (selectedCategory !== null) {
    ...
    const categoryCommands = smartleadModule.commands.filter(cmd => 
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Box flexDirection="column" marginBottom={1}>
        ...
        <Text color="gray">
          • Categories: {smartleadModule.categories.length}
        </Text>
        <Text color="gray">
          • Status: ✅ Active
        </Text>
      ...
      <Box flexDirection="column" marginBottom={1}>
        ...
        {smartleadModule.categories.map((category, index) => (
